"""
EWD Copies APAC Data Ingestion Script

This script extracts EWD Copies APAC data from Monday.com and saves it to S3/Unity Catalog volumes.
...
def main():
    """Main entry point for EWD Copies APAC ingestion."""
    ...
    # Hardcoded parameters for EWD Copies APAC
    board_id = "4440469490"
    group_id = "topics"
    vertical = "dismis"
    data_domain = "trends"
    file_name_prefix = "ewd_copies_apac"

    # Get Monday.com API key from Databricks secrets
    monday_api_key = dbutils.secrets.get(scope="hoangl", key="monday_api_key")

    # EWD Copies APAC specific columns (placeholder - will be updated later)
    include_columns = [
        "name",
        "geo",
        "date_reported",
