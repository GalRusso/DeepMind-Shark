"""
EWD Trends APAC Data Ingestion Script

This script extracts EWD Trends APAC data from Monday.com and saves it to S3/Unity Catalog volumes.
...
def main():
    """Main entry point for EWD Trends APAC ingestion."""

    # Parse command line arguments
    parser = argparse.ArgumentParser(description="EWD Trends APAC Data Ingestion")
    parser.add_argument(
        "--environment",
        required=True,
        choices=["dev", "prod"],
    ...
    # Hardcoded parameters for EWD Trends APAC
    board_id = "4440702028"
    group_id = "topics"
    vertical = "dismis"
    data_domain = "trends"
    file_name_prefix = "ewd_trends_apac"

    # Get Monday.com API key from Databricks secrets
    monday_api_key = dbutils.secrets.get(scope="hoangl", key="monday_api_key")

    # EWD Trends APAC specific columns (placeholder - will be updated later)
    include_columns = [
        "name",
        "external_trend_id",
