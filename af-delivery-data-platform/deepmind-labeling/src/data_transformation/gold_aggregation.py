...
def gold_images():
    ...
    # Use coalesce to replace null abuse fields with empty arrays (consistent with video gold)
    gold = gold.select(
        ...
        coalesce(col("ab.abuse_categories"), array().cast(ArrayType(StringType()))).alias("abuse_categories"),
        coalesce(col("ab.abuse_categories_conf"), array().cast(ArrayType(IntegerType()))).alias("abuse_categories_conf"),
        coalesce(col("ab.abuse_categories_reasoning"), array().cast(ArrayType(StringType()))).alias("abuse_categories_reasoning"),
        coalesce(col("ab.adversary_levels"), array().cast(ArrayType(StringType()))).alias("adversary_levels"),
        coalesce(col("ab.adversary_levels_conf"), array().cast(ArrayType(IntegerType()))).alias("adversary_levels_conf"),
        coalesce(col("ab.adversary_levels_reasoning"), array().cast(ArrayType(StringType()))).alias("adversary_levels_reasoning"),
