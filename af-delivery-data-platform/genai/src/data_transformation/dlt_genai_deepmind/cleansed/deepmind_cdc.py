...
schema = StructType([
    ...
    StructField("target_adversarial_level", StringType(), True, metadata={"comment": "The level of severity of this prompt's violation"}),
    StructField("additional_context", StringType(), True, metadata={"comment": "Further information/context that was delivered to the client that don't qualify to another columns"}),
...
dlt.create_streaming_table(
    ...
    comment="Cleansed Deepmind data with standardized formats and data quality checks",
    schema=schema,
    table_properties={
        "quality": "silver",
...
@dlt.append_flow(
    ...
    comment="Cleansed Deepmind data with standardized formats and data quality checks flow",
)
def landing_to_cleansed_cdc_flow():
    """
    Silver layer: Clean and standardize Deepmind data from landing layer.
    ...
    return (
        ...
        .select(
            ...
            trim(col("target_adversarial_level")).alias("target_adversarial_level"),
            trim(col("additional_context")).alias("additional_context"),
