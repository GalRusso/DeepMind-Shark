...
def dir_path(kind: Literal["image", "audio", "video"]):
    ...
    return f"/Volumes/af_delivery_{ENVIRONMENT}/data_collection/{VERTICAL}/{kind}/{SOURCE}/{now:%Y}/{now:%m}/{now:%d}"
    ...
    for result, sheet_id, source_id, file_name, gdrive_path in zip(
        ...
        try:
            ...
            with io.BytesIO(content) as content_io:
                workspace_client.files.upload(file_path=dest_path, contents=content_io, overwrite=True)

            with io.BytesIO(json.dumps(metadata).encode("utf-8")) as metadata_io:
                workspace_client.files.upload(file_path=meta_path, contents=metadata_io, overwrite=True)
