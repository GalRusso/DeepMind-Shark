...
def processor(items: list[dict]) -> list[ProcessingResult]:
    ...
    processor = VideoDescriptionTagProcessor(
        ...
        logger=logger,
        gemini_api_key=dbutils.secrets.get(scope="datn_labeling", key="gemini_api_key"),
    )
...
if __name__ == "__main__":
    ...
    spark.sql("DELETE FROM af_delivery_dev.silver_labeling.tasks_status where task_key = 'video.des_tag';")
    spark.sql("DELETE FROM af_delivery_dev.silver_labeling.tasks_todo where task_key = 'video.des_tag';")
    spark.sql("DELETE FROM af_delivery_dev.silver_labeling.tasks_results where task_key = 'video.des_tag';")
